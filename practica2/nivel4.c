...
int execute_line(char *line)
{
  ...
  if (!check_internal(tokens))
  {
    ...
    if (pid == 0) // Proceso hijo
    {
      ...
      execvp(tokens[0], tokens); // execvp no retorna si ha ido bien la operacion
      fprintf(stderr, GRIS_T "[No se encontro la orden]");
      // perror("No se encontro la orden"); SE PUEDE BORRAR
      exit(FAILURE);
    }
